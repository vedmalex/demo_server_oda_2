...
export default class Person
  ...
  public async addToCreatedBy(args: { person?: string; user?: string }) {
    logger.trace(`addToCreatedBy`);
    let opposite = await this.connectors.User.findOneById(args.user);
    if (opposite) {
      await this.findOneByIdAndUpdate(args.person, { createdBy: opposite.id });
    }
  }

  public async removeFromCreatedBy(args: { person?: string; user?: string }) {
    logger.trace(`removeFromCreatedBy`);
    await this.findOneByIdAndUpdate(args.person, { createdBy: null });
  }

  public async addToUpdateBy(args: { person?: string; user?: string }) {
    logger.trace(`addToUpdateBy`);
    let opposite = await this.connectors.User.findOneById(args.user);
    if (opposite) {
      await this.findOneByIdAndUpdate(args.person, { updateBy: opposite.id });
    }
  }

  public async removeFromUpdateBy(args: { person?: string; user?: string }) {
    logger.trace(`removeFromUpdateBy`);
    await this.findOneByIdAndUpdate(args.person, { updateBy: null });
  }
